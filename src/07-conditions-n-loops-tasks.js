...
function getCommonDirectoryPath(pathes) {
  ...
  while (isSame) {
    for (let i = 0; i < pathes.length; i += 1) {
      if (pathes[0][current] !== pathes[i][current]) isSame = false;
    }

    if (isSame) {
      if (pathes[0][current] === '/') breakPoint = current + 1;
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < m; i += 1) {
    for (let j = 0; j < n; j += 1) ans[i][j] = 0;
  }
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < m; j += 1) {
      for (let k = 0; k < m1[0].length; k += 1) ans[i][j] += m1[i][k] * m2[k][j];
    }
  }
  return ans;
}
...
function evaluateTicTacToePosition(position) {
  ...
  let fl1 = true;
  let fl2 = true;
  for (let x = 1; x < position.length; x += 1) {
